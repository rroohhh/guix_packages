...
(define-public scrabble-server
  (package
    ...
    (source
     (origin
       (method url-fetch)
       (uri (string-append "https://files.niemo.de/ScrabbleServer_V0.23_B120_D202010171549"))
       (sha256
     ...
     `(#:modules ((guix build utils))
       ...
       (begin
         ...
         (let* ((bin (string-append %output "/bin"))
           ...
           (copy-file (assoc-ref %build-inputs "source") "ScrabbleServer")
           (install-file "ScrabbleServer" bin)
           (chmod (string-append bin "/ScrabbleServer") #o755)
           (invoke patchelf "--set-interpreter" ld-so (string-append bin  "/ScrabbleServer")))
         #t)))
    (home-page
      "https://git.froheiyd.de/dirk/Scrabble")
    (synopsis "Multiplayer cross platform scrabble server in go")
    (description
      "Multiplayer cross platform scrabble server in go")
